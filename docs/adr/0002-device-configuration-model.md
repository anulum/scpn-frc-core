...
SCPN FRC Core — ADR 0002: device configuration model
-->
...
(`field_reversed_configuration`). The claim boundary and
repository-level `evidence_maturity` semantics follow the family pilot.

## Decision

1. The package `scpn_frc_core` implements the device configuration
   model as frozen, strictly typed value objects: separatrix/coil
   geometry and operational limits.
...
3. Hard invariant: the separatrix radius is strictly smaller than the
   coil radius (``x_s = r_s / r_c`` in ``(0, 1)``) — the separatrix of
   a field-reversed configuration lies inside its confinement coil.
4. Derived quantity with citation: the volume-averaged beta follows the
   FRC average-beta relation ``<beta> = 1 - x_s^2 / 2``
   (M. Tuszewski, Nucl. Fusion 28 (1988) 2033). Advisory findings,
   reported by `consistency_report()` and never clamped: elongation
   ``E = l_s / (2 r_s)`` below one (FRC equilibria are prolate;
   Tuszewski 1988) and ``x_s >= 0.95`` (separatrix within five percent
   of the coil radius).
...
- Later lanes (excluded-flux diagnostic semantics, safety envelope)
