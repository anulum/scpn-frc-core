...
fusion systems within the SCPN Reactor Systems Research Group. This
repository is the designated owner of device-level truth for the
`field_reversed_configuration` configuration of the SCPN Phase Orchestrator
...
serialisation, and a data-only SPO registry pin (`src/scpn_frc_core/`,
...
This repository owns, for the FRC device family:

- the device boundary: plant and experiment truth, shot lifecycle, and
  configuration policy for elongated compact toroids confined almost
  entirely by poloidal field, with negligible toroidal field, closed-field
  core inside a separatrix embedded in an open-field scrape-off region, and
  volume-averaged beta near unity;
- FRC equilibrium, transport, and stability physics **as device truth**
  (parameter envelopes, regime identification, stability boundaries such as
  the internal tilt and rotational modes and their kinetic stabilisation
  margins) — while the corresponding solver mathematics remains with the
  solver owner;
- formation and sustainment semantics at device level (field-reversed
  theta-pinch class formation, rotating-magnetic-field current drive,
  neutral-beam sustainment) as configuration facets;
- diagnostic semantics, reference frames (separatrix-relative labels,
  excluded-flux reconstruction), and clock identity declarations;
...
- **Pulsed FRC/MIF merge-compression workflow, trigger decision, and RTL**:
  `SCPN-MIF-CORE` (accepted existing owner). This repository owns FRC
  physics truth; the merge/compression integration lifecycle stays there.
- **Plasma-jet liner compression**: `SCPN-MIF-PLASMA-JET-CORE`.
- **Spheromak** (comparable toroidal field, Taylor relaxation):
  `SCPN-SPHEROMAK-CORE`.
- **Theta-pinch compression devices**: `SCPN-THETA-PINCH-CORE`.
- **Solver mathematics and validation evidence**: `SCPN-FUSION-CORE` —
  explicitly including its current FRC solver surfaces — until an exact
  surface passes the reactor family migration gate. No solver code exists
  in, or was copied into, this repository.
...
artefact, and no parameter set describes or validates any real machine. Formation-method and fuel-cycle choices (including
aneutronic ambitions) are configuration facets, not separate claims. No capability has reached any
